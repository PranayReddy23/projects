...
main = tkinter.Tk()
main.title("DEEP LEARING POWERED ARRHYTHMIA DIAGNOSIS AND TREATMENT WITH NOISE DETECTION")
...
def runCNN():
    ...
    if os.path.exists('model/cnn_model.json'):
        ...
        cnn = Sequential()
        cnn.add(Convolution2D(256, 1, 1, input_shape = (X_train.shape[1], X_train.shape[2], X_train.shape[3]), activation = 'relu'))
        ...
        hist = cnn.fit(X_train, y_train, batch_size=32, epochs=5, shuffle=False, verbose=1, validation_data=(X_test, y_test))
