...
def predict_with_cnn():
    global cnn, pca  # Assuming pca is the trained PCA model
    data_string = input_entry.get()
    data_list = data_string.split(", ")
    if len(data_list) != 279:
        text.insert(END, "Invalid data: 279 values required.\n")
        return
    try:
        # Convert data to float
        new_data_array = np.array(data_list, dtype=float)
        # Transform data into 40 principal components
        transformed_data = pca.transform(new_data_array.reshape(1, -1))
        # Reshape transformed data to match CNN input shape
        transformed_data_reshaped = transformed_data.reshape(1, 40, 1, 1)
        # Make predictions using the CNN model
        predicted_labels = cnn.predict(transformed_data_reshaped)
...
Enter new data in sequential (comma separated, 279 values required):""")
